...
const getRadioList = (country, genre) => {
  fetch(`http://de1.api.radio-browser.info/json/stations/bycountry/${country}/bytag=?&{genre}`)
    .then((response) => {
      console.log(response.status);
      if (!response.ok)
        throw new Error(`Ошибка: ${response.status}`);
    ...
    .then((stations) => {
      ...
      stations.forEach((station) => {
        ...
        listItem.addEventListener("click", () => {
          window.open(`https://en.wikipedia.org/wiki/${station.name}`);
        });
...
//   radio_list.innerHTML = "";
//   stations.forEach((station) => {
//     let li = `<li><a href="${station.url}" target="_blank">${station.name}</a></li>`;
...
radio_list.insertAdjacentHTML("beforeend", li);
document.querySelector(".color").style.marginLeft = "50px";



// https://de1.api.radio-browser.info/json/stations/bytag?tags=${genre}&country=${country}
...
const stations = () => {
  fetch(`https://de1.api.radio-browser.info/json/stations/bycountry/`)
  radio_list.innerHTML = "";
  stations.forEach((station) => {
    let listItem = document.createElement("li");
    listItem.textContent = station.name;
